...
module.exports = function handler(event, context) {
  ...
  logger.debug('Lambda-formation project entrypoint', {
    event:   event,
    context: context
  });
...
//  if the type include - then we have a folder structure
  if(resourceType.includes('-')) {
    //create the relative path
    dirPath          = _.replace(resourceType, '-', '/');
    // the resource path, or where to find the rescues
    resourceTypePath = path.dirname(dirPath);
    // get the type of the resource from the path
    resourceType     = path.basename(dirPath);
    logger.debug(`The dir full path ${dirPath}
                resourceTypePath: ${resourceTypePath}
                resourceType: ${resourceType}`);
  }
  var caller = callerId.getData();
  
  var resourcePath = path.join(caller.filePath, '..', 'lib', 'resources', resourceTypePath);
